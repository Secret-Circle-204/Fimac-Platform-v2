...
class CustomValidationError extends Error {
  ...
  constructor(message: string) {
    ...
    this.status = 400;
    this.data = { errors: [{ message, field: 'location.geo.lat' }, { message, field: 'location.geo.lng' }] };
  }
}

export const formatAddress: CollectionAfterReadHook<Property> = measureAfterRead('formatAddress', async ({ doc }) => {
  const location_legacy = typeof doc.location_legacy === 'object' && doc.location_legacy !== null ? doc.location_legacy : null
  ...
  const full_address = doc.location?.address?.fullAddress || `${street}, ${city}, ${state_abbr} ${zip}`
...
export const syncLocationHook: CollectionBeforeChangeHook<Property> = measureBeforeChange('syncLocationHook', async ({ data, req, operation }) => {
  try {
    // DEBUG LOGGING - Auditing Auth Context
    console.log(`[PropertiesHook] Operation: ${operation}, Collection: properties, User: ${req.user ? `${req.user.email} (${req.user.collection})` : 'UNDEFINED'}`)
      ...
      if (resolved.geo && isValidCoordinate(resolved.geo.lat, resolved.geo.lng)) {
        ...
        if (resolved.address) {
          ...
          if (geocodedAddress) {
            ...
            if (hasArabic && resolved.streetHint && !/[\u0600-\u06FF]/.test(resolved.streetHint)) {
              data.location.address.street = resolved.streetHint
            }
          }
        }
      } else {
        console.warn(`[PropertiesHook] Smart Location Helper failed to resolve input: "${data.mapsUrlInput}". Error: ${resolved.error}`)
      }
    }
    ...
    if (coordsModified && latVal !== undefined && lngVal !== undefined) {
      ...
      if (latVal > 31.8 && latVal <= 37.0 && lngVal >= 22.0 && lngVal <= 31.8) {
        console.log(`[PropertiesHook] Swapping coordinates to correct admin error: (${latVal}, ${lngVal}) -> (${lngVal}, ${latVal})`)
        const temp = latVal
        latVal = lngVal
        lngVal = temp
    ...
    if (data.location?.geo) {
      ...
      if (lat !== undefined && lng !== undefined && lat !== null && lng !== null) {
        if (!isValidCoordinate(lat, lng)) {
          throw new CustomValidationError(`Invalid Geolocation: Latitude (${lat}) and Longitude (${lng}) are not valid coordinates.`)
        }
      }
    }
    ...
    if (data.location?.address) {
      ...
      if (street || city || state) {
        data.location.address.fullAddress = buildFullAddress(street, city, state, zip || undefined)
    ...
    req.payload.logger.error(`[PropertiesHook] CRITICAL ERROR in syncLocationHook: ${error instanceof Error ? error.message : 'Unknown'}`)
...
export const revalidatePropertyCache: CollectionAfterChangeHook<Property> = async ({ doc, previousDoc }) => {
  try {
...
export const revalidatePropertyDeleteCache: CollectionAfterDeleteHook<Property> = async ({ doc }) => {
  try {
    const id = doc?.id
    if (id) {
      triggerRevalidate(`property:${id}`)
