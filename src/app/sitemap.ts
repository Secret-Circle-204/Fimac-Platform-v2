...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
  ...
  try {
    ...
    blogEntries = blogResult.docs
      .filter((doc) => typeof doc.slug === 'string' && doc.slug.length > 0)
      .map((doc) => ({
        url: `${SERVER_URL}/blog/${doc.slug}`,
        lastModified: doc.updatedAt ? new Date(doc.updatedAt) : new Date(),
      }))
  } catch (error) {
    console.error('Error compiling blog posts for sitemap:', error)
  }
  ...
  try {
    ...
    console.error('Error compiling properties for sitemap:', error)
  }
